...
CONFIG = {
    ...
    "SCHEMA":  get_env("DBX_SCHEMA",  "invetory_sales_schema"),

    # Table names inside that schema (change if yours differ).
    "SALES_TABLE":     get_env("DBX_SALES_TABLE",     "sales_data_updated"),
    "INVENTORY_TABLE": get_env("DBX_INVENTORY_TABLE", "inventory_data_curated"),
}
...
def api_predict():
    """
    Forecast endpoint (TabPFN spec, Option C). Body:
      { target, scope, entity, client, horizon }
    Runs Planner -> Feature Builder -> TabPFN Service -> Synthesis against the
    same Databricks tables the dashboard uses.
    """
    from predict import run_prediction
    req = request.get_json(force=True) or {}
    try:
        result = run_prediction(req, run_sql, fqtn)
        return jsonify(result)
    except ValueError as e:
        # validation / no-data problems are user-facing, not server errors
...
# ---------------------------------------------------------------------------
#  Option B  —  unified query endpoint (Planner routes retrieval vs prediction)
# ---------------------------------------------------------------------------
_ENTITY_CACHE = {"data": None}


def get_entities():
    """
    Lightweight list of known client + ship-to names for the Planner to resolve
    against. Cached for the process lifetime; a small dedicated query so we don't
    rebuild the full dashboard payload on every question.
    """
    ...
    try:
        rows = run_sql(f"""
            SELECT DISTINCT s.`Sold-To_Party3` AS client,
                            s.`Ship-to_party14` AS shipto
            FROM {sales} s
            WHERE s.`Sold-To_Party3` IS NOT NULL
        """)
        clients = sorted({r["client"] for r in rows if r.get("client")})
        shiptos = [{"name": r["shipto"], "client": r["client"]}
                   for r in rows if r.get("shipto")]
        # dedupe ship-tos by name
        seen, uniq = set(), []
        for s in shiptos:
            if s["name"] not in seen:
                seen.add(s["name"]); uniq.append(s)
        _ENTITY_CACHE["data"] = {"clients": clients, "shiptos": uniq}
    except Exception as e:
        log.warning("entity list failed (%s); planner will see no entities", e)
        _ENTITY_CACHE["data"] = {"clients": [], "shiptos": []}
    return _ENTITY_CACHE["data"]
...
def api_query():
    """
    Unified 'ask anything' endpoint (Option B). Classifies the question via the
    Planner, then routes:
      retrieval  -> genie_ask()        (existing)
      prediction -> run_prediction()   (existing Option C chain)
    Returns one consistent envelope so the front-end renders both uniformly.
    """
    from planner import classify
    from predict import run_prediction
    from synthesis import unify
    ...
    # 1. Plan
    try:
        plan = classify(q, get_entities())
    except Exception as e:
        log.exception("planner failed")
        plan = {"task": "retrieval", "confidence": 0.0, "method": "error"}

    routing = {"task": plan.get("task"), "confidence": plan.get("confidence"),
               "method": plan.get("method")}

    # 2. Route -> 3. Unify (one envelope) -> 5. self-check (inside unify)
    try:
        if plan.get("task") == "prediction":
            req = {k: plan.get(k) for k in ("target", "scope", "entity", "client", "horizon")}
            try:
                result = run_prediction(req, run_sql, fqtn)
                return jsonify(unify(q, "prediction", result, routing))
            except ValueError as ve:
                # Prediction couldn't resolve (unknown entity / no history).
                # Fall back to retrieval so the user still gets an answer.
                log.info("prediction fallback -> retrieval: %s", ve)
                routing["fallback"] = f"prediction unavailable: {ve}"
                g = genie_ask(q)
                return jsonify(unify(q, "retrieval", g, routing))
        else:
            # Guardrail the question before it reaches Genie.
            verdict = guardrail_check(q)
            if verdict["blocked"]:
            ...
            if g.get("sql"):
                track_queried_tables(g["sql"], q, len(g.get("rows", [])), "Genie")
            g["guardrail"] = verdict
            return jsonify(unify(q, "retrieval", g, routing))
    except Exception as e:
        log.exception("query routing failed")
        return jsonify({"error": str(e), "routing": routing}), 500

##################DASHBOARD#############################
INSIGHTS_TABLE = get_env("DBX_INSIGHTS_TABLE", "ernest_insights")


DASH_LIVE = LIVE
# In-memory fallback store (also used to hold rows fetched for Delta insights).
_INSIGHTS = []
_insights_lock = threading.Lock()
_next_id = [1]


def _new_insight_id() -> int:
    with _insights_lock:
        i = _next_id[0]
        _next_id[0] += 1
        return i
...
def insights_fqtn() -> str:
    return fqtn(INSIGHTS_TABLE)


def _esc(v, n=2000) -> str:
    return str(v if v is not None else "").replace("'", "''")[:n]


def list_insights(persona: str = "", include_shared: bool = True):
    """Return saved insights. In live mode, read from Delta (own + shared);
    otherwise return the in-memory store. Rows are NOT run here — the caller
    hydrates each card's data via /api/dashboard/insight/run."""
    if DASH_LIVE:
        try:
            where = "1=1"
            if persona:
                p = _esc(persona, 200)
                where = f"created_by = '{p}'" + (" OR is_shared = true" if include_shared else "")
            sql = f"""SELECT insight_id, title, question_text, sql_query, chart_type,
            ...
            for r in rows:
                out.append({
                    "id": r.get("insight_id"),
                    "delta_id": r.get("insight_id"),
                    "title": r.get("title") or "Untitled",
                    "question": r.get("question_text") or "",
                    "sql": r.get("sql_query") or "",
                    "chart_type": r.get("chart_type") or "table",
                    "persona": r.get("created_by") or "—",
                    "created_at": str(r.get("created_at") or "")[11:16] or "—",
            ...
            log.warning("[dashboard] Delta list failed, using memory: %s", e)
    with _insights_lock:
        return [dict(i) for i in _INSIGHTS]


def save_insight(body: dict):
    """Persist a new insight. Writes to Delta in live mode and always keeps a
    copy in memory so it renders immediately."""
    insight = {
        "id": _new_insight_id(),
        "delta_id": None,
        "title": (body.get("title") or "Untitled")[:200],
        "question": (body.get("question") or "")[:400],
        "sql": (body.get("sql") or "")[:2000],
        "chart_type": body.get("chart_type") or "table",
        "persona": body.get("persona") or "Analyst",
    ...
    if DASH_LIVE:
        try:
            sql = f"""INSERT INTO {insights_fqtn()}
                (title, question_text, sql_query, chart_type, created_by, is_shared, created_at)
                VALUES ('{_esc(insight['title'],200)}', '{_esc(insight['question'],400)}',
                        '{_esc(insight['sql'])}', '{_esc(insight['chart_type'],40)}',
                        '{_esc(insight['persona'],200)}', {str(insight['is_shared']).lower()},
                        current_timestamp())"""
            run_sql(sql)
        except Exception as e:
            log.warning("[dashboard] Delta save failed (kept in memory): %s", e)
    return insight


def delete_insight(ins_id):
    """Remove an insight from memory and (if live) from Delta."""
    delta_id = None
    with _insights_lock:
        for i in _INSIGHTS:
            if str(i["id"]) == str(ins_id):
                delta_id = i.get("delta_id")
                break
        _INSIGHTS[:] = [i for i in _INSIGHTS if str(i["id"]) != str(ins_id)]
    target = delta_id if delta_id is not None else ins_id
    if DASH_LIVE and target is not None:
        try:
            run_sql(f"DELETE FROM {insights_fqtn()} WHERE insight_id = {int(target)}")
        except Exception as e:
            log.warning("[dashboard] Delta delete failed: %s", e)
    return True


def update_insight_chart(ins_id, chart_type: str):
    """Persist a card's chart-type preference."""
    delta_id = None
    with _insights_lock:
        for i in _INSIGHTS:
            if str(i["id"]) == str(ins_id):
    ...
    if DASH_LIVE:
        target = delta_id if delta_id is not None else ins_id
        try:
            run_sql(f"UPDATE {insights_fqtn()} SET chart_type = '{_esc(chart_type,40)}' "
                    f"WHERE insight_id = {int(target)}")
        except Exception as e:
            log.warning("[dashboard] chart_type update failed: %s", e)
    return True


def run_insight_sql(sql: str):
    """Execute an insight's stored SQL to hydrate its card. Returns rows (live)
    or a small simulated set so the UI still renders offline."""
    if DASH_LIVE and sql:
        try:
            return {"rows": run_sql(sql)[:200], "live": True}
        except Exception as e:
            log.warning("[dashboard] insight run failed: %s", e)
            return {"rows": [], "live": False, "error": str(e)}
    return {"rows": _simulate_rows(sql), "live": False}


def _simulate_rows(sql: str):
    """Tiny deterministic sample so cards render without a warehouse."""
    return [
        {"category": "Motor", "revenue": 612000, "gtm_pct": 24.1},
        {"category": "Property", "revenue": 431000, "gtm_pct": 31.6},
        {"category": "Liability", "revenue": 388000, "gtm_pct": 18.2},
        {"category": "Marine", "revenue": 205000, "gtm_pct": 27.9},
    ]
...
def api_dashboard_insights():
    ...
    try:
        return jsonify({"insights": list_insights(persona), "live": LIVE})
    except Exception as e:
        log.exception("dashboard list failed")
...
def api_dashboard_save():
    body = request.get_json(force=True) or {}
    if not (body.get("title") and body.get("sql")):
        return jsonify({"error": "title and sql are required"}), 400
    return jsonify({"insight": save_insight(body), "live": LIVE})


@app.route("/api/dashboard/insights/<ins_id>", methods=["DELETE"])
def api_dashboard_delete(ins_id):
    delete_insight(ins_id)
    return jsonify({"deleted": True})


@app.route("/api/dashboard/insights/<ins_id>/chart", methods=["POST"])
def api_dashboard_chart(ins_id):
    ct = (request.get_json(force=True) or {}).get("chart_type", "table")
    update_insight_chart(ins_id, ct)
    return jsonify({"updated": True, "chart_type": ct})


@app.route("/api/dashboard/insight/run", methods=["POST"])
def api_dashboard_run():
    sql = (request.get_json(force=True) or {}).get("sql", "")
    return jsonify(run_insight_sql(sql))
